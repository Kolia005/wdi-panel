...
export async function load() {
	...
	return {
		...
		mappings: mappings.map((m) => ({ id: String(m._id), wixName: m.wixName, productName: m.productName, productId: String(m.productId) })),
...
export const actions = {
	...
	saveMapping: async ({ request }) => {
		...
		if (!wixName || !productId) return fail(400, { ok: false, message: 'Wix name and product required.' });
		const database = await db();
		const { products } = await collections();
		...
		await database.collection('productmappings').updateOne(
			{ wixName },
			{ $set: { wixName, productId: product._id, productName: product.name, at: new Date() } },
			{ upsert: true }
		);
		await audit('webstore.mapping', { wixName, product: product.name });
		return { ok: true, message: `Mapped "${wixName}" → ${product.name}.` };
	},
